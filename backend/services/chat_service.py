...
OFF_TOPIC_REPLY = (
    "That question is **not suitable for this dataset**. I only help with the **Care-Sync** "
    "wearable **sensor** table, stress **labels**, and what you see on the **dashboard** "
    "(heatmap, trends, anonymized caregiver IDs). Ask something about those and I will answer."
)
...
def chat_reply(message: str, ui_state=None, heatmap_cell=None):
    ...
    if _is_critical_heatmap_cell(hc):
        return build_critical_stress_narrative(hc)
    ...
    if gemini_key:
        try:
            ...
            return provider_prefix + _gemini_reply(message, context)
        except Exception as exc:
            if _is_quota_or_rate_limit(exc):
                return (
                    ...
                    "---\n\n"
                    + _fallback_conversational(message, context)
                )
            return (
                f"[Gemini error: {exc}]\n\n"
                + _fallback_conversational(message, context)
    ...
    if openai_key:
        try:
            ...
            return provider_prefix + _openai_reply(message, context)
        except Exception as exc:
            return (
                f"[OpenAI error: {exc}]\n\n" + _fallback_conversational(message, context)
            )
    ...
    return provider_prefix + _fallback_conversational(message, context)
...
def _fallback_conversational(message: str, context: str) -> str:
    ...
    intro = f"You asked: **{message.strip()[:280]}**\n\n"
...
def _offline_reply(message: str, context: str):
    ...
    if any(k in m for k in ("hello", "hi ", "hey", "thanks", "thank you")):
        return (
            "Hi — the cloud model is unavailable or over quota, so I am answering from **rules + your live dataset summary** below. "
            "Ask about stress, months, or signals and I will stay grounded in this context.\n\n"
            f"{context}"
        )
    if any(k in m for k in ("name", "who is", "role", "department")):
        return (
            "The dashboard intentionally hides names and roles. "
            "Open **Caregiver Details** for profile fields; here is what the dashboard "
            f"can say from sensor data only:\n\n{context}"
        )
    if any(k in m for k in ("factor", "influence", "correlate", "important")):
        return (
            "From the sensor features available, **EDA** and **HR** are the strongest "
            "cues aligned with stress labels in this dataset; **MovementMagnitude** and "
            "**TEMP** add context (activity vs. rest, physiology). "
            f"Use the correlation and box plots to compare.\n\n{context}"
        )
    if any(k in m for k in ("trend", "anomal", "spike", "pattern")):
        return (
            "Check the time-series and monthly coverage chart: sparse months reflect "
            "fewer caregivers contributing readings. Compare anonymized ids side by side.\n\n"
            f"{context}"
        )
    if any(
        k in m
        ...
        return (
            "**Evidence from your live database (offline mode):**\n\n"
            "Use the figures below—they are computed from `sensor_data` right now. "
            "Then ask a follow-up (e.g. one anonymized ID or one month).\n\n"
            f"{context}"
        )
    return (
        "Try asking how EDA/HR relate to stress, which months have fewer caregivers, "
        "or what the current filters show. "
        f"Summary:\n\n{context}"
    )
